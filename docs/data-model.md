...
Do not retain a complete page DOM, cookies, scripts, tracking pixels, unrelated page text, or browser history. The manual path accepts plain employer-authored text, normalizes Unicode/newlines and trailing whitespace, creates escaped paragraph/line-break HTML, and hashes the canonical normalized plain text as 64 lowercase hexadecimal SHA-256 characters. Milestone 2A's extension path accepts reviewed semantic HTML/text and extraction metadata, then the server independently validates URLs, sanitizes to an allowlist of semantic tags with no attributes, normalizes text, and computes the authoritative SHA-256 before one transactional application/event/snapshot insert. Employment type and salary text are non-authoritative capture metadata in 2A, not new application columns. The semantic snapshot definition is confirmed by C-014 in `docs/decisions.md`; screenshots, full-page archives, duplicate attachment, and snapshot recapture UI are postponed.
...
- Milestone 1B-2 enabled and forced RLS on `owners` and all ten `owner_id` tables. `owners` is visible only when its Clerk subject matches `auth.user_id()`; child policies compare `owner_id` with `wip_current_owner_id()`.
- Milestones 1B-3 and 1C retain the passwordless `authenticated` role as `NOBYPASSRLS` and add owner-matching INSERT/UPDATE/DELETE policies only where commands need them. Applications, notes, next actions, logical documents, contacts, and their associations receive only relevant mutable operations; events, snapshots, and document versions receive INSERT but no UPDATE/DELETE. Column-level grants further restrict writable fields. Whole-tracker deletion is exposed only through an owner-derived zero-argument function rather than broad child-table delete grants.
...
- `DATABASE_URL` is privileged fictional-seed tooling, `DIRECT_DATABASE_URL` is privileged migration tooling, and `NEON_AUTHENTICATED_DATABASE_URL` is normal request-time read/write access through the JWT-authenticated role. Owner/migration credentials are never used for real-user requests or exposed to browsers.
...
After Neon RLS provisioning supplies the JWT helper and `authenticated` role, migration `0002_clerk_auth_rls.sql` verifies that the role exists without `BYPASSRLS`, defines Wip's functions/read policies/grants, creates the Clerk-subject uniqueness constraint, and forces RLS. Migrations `0003_silky_the_fallen.sql` and `0004_broad_tombstone.sql` add mutation/projection/idempotency/version fields, owner-matching write policies, checks, and narrow grants. Migration `0005_milestone_1c_tracker.sql` adds contact/document taxonomies, metadata versions, policies/grants, and the owner-derived tracker-deletion function; `0006_tracker-deletion-settings.sql` extends that function to reset tracker preferences while retaining the auth mapping. The command layer uses request-local JWT-authenticated Neon/Drizzle clients; fixed multi-record commands run as HTTP batch transactions behind repository/command boundaries rather than from React or route handlers.

`event_confirmation_decisions`, aggregation consent, inbox processing, and analytics schemas remain conceptual later work and are deliberately absent. Milestone 2A required no database migration: the existing `extension` event/snapshot source values, immutable snapshot columns, owner-scoped application create idempotency, and narrow insert grants already support the capture command. Snapshot recapture/duplicate attachment, archive/restore, file contents, and Clerk-account deletion remain absent.
